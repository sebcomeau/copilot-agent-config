...
1. Confirm preconditions — Identify the repository root, implementation handoff, exact command, validation boundary, complete affected-test manifest, and concurrency plan. If a command or manifest is missing, stop as blocked and request it.
...
3. Execute — Run every focused selector in the assigned shard without editing files. When instructed and supported safely by the test runner, use up to three workers. Prefer test-file, test-class, package, or equivalent targeted selectors (for example, Gradle `test --tests ...`) over a whole-suite command. A shard may contain multiple selectors; three is the concurrency limit, not the number of tests that must be covered.
4. Classify — Report pass or failure for every assigned selector. For failures, state whether the evidence suggests an implementation regression, test issue, pre-existing failure, flaky behavior, or environment problem. Mark uncertainty explicitly.
5. Hand off — Return the concise report below so the orchestrator can either accept the result or resume the original implementer. Include skipped selectors and whether reviewer handoff is unblocked.

## Report format (strict)

- Scope — repository root, command, validation boundary, worker or shard assignment, and the complete affected-test manifest with coverage status.
