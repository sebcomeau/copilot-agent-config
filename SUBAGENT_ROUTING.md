...
- Detach behavioral verification from `implementer` by default. After implementation and cheap structural checks, keep the original implementer available and delegate focused test, build, lint, type-check, parsing, or documentation checks to `code-validator`. Every validator prompt must state the exact command, validation boundary, assigned target, and whether execution is sequential or concurrent. For test validation, build a complete affected-test manifest from every added or changed test file plus directly affected existing tests, then run every entry with test-file, test-class, package, or equivalent selectors instead of substituting a whole-suite command; for example, use Gradle `test --tests ...` selectors. For non-test validation, provide the applicable file, package, or check selector and mark the affected-test manifest as `not applicable`. Require a concurrency plan only for concurrent execution. Prefer one validator using up to three test-runner workers when supported and concurrency-safe. Otherwise partition the complete test manifest across up to three `code-validator` agents with distinct, non-overlapping shards; each shard may contain multiple test selectors. Three limits concurrent workers or agents, not the number of affected tests that must run. Do not parallelize commands that share mutable databases, fixtures, snapshots, generated files, ports, caches, or coverage outputs unless those resources are isolated.
...
Every handoff states each applicable field below. Use `not applicable` with a reason for fields outside the delegated role. A discovery handoff may mark a field `unresolved` only when resolving it is part of the reported evidence gap; identify where to find it and which downstream role owns resolution.
...
- Exact command or test selectors for validation when repository evidence establishes them; otherwise the resolution method and owning downstream role
